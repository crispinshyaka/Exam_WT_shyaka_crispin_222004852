...
$sql = "SELECT * FROM consultations WHERE consultant_id = 0";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $consultant_id = $row['refferenced_id'];

    // Update consultation in the database for the selected case
    $sql = "UPDATE consultations SET consultant_id = '$consultant_id', session_note = '$consultation' WHERE client_id = '$client_id' AND consultant_id = 0";
...
<body>
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <label for="session_note">Choose a case:</label>
        <select name="client_id" id="session_note">
            <?php
            // Display available cases in dropdown
            while ($row = mysqli_fetch_assoc($result)) {
                echo "<option value='" . $row['client_id'] . "'>" . $row['session_note'] . "</option>";
            }
            ?>
        </select><br><br>
        <label for="session_note">Consultation:</label><br>
        <textarea name="session_note" id="session_note" rows="4" cols="50"></textarea><br><br>
